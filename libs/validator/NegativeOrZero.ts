...
import {
  MetadataKey_NegativeOrZero as MetadataKey,
  MetadataKey_NegativeOrZeroList as MetadataKeyList,
...
function validate(param: any, decoratorData: any): { r?: boolean; v?: any } {
  ...
  if (febs.utils.bigint_more_than(param, 0)) {
    return { r: false }
  }
 ...
 * 验证是否是数组.
 */
NegativeOrZero.List = DecoratorList
...
export function NegativeOrZero(
  target: Object,
  propertyKey: string | symbol
): void
export function NegativeOrZero(...args: any[]) {
  if (args.length == 1 || args.length == 0) {
    let cfg: any = args[0] || {}
    return (target: Object, propertyKey: string | symbol) => {
      _validate_set_property_matedata(
